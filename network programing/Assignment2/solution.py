import json, logging, ssl, socket, threading, zlib, argparse
...
def client(host, port, cafile=None):
    ...
    try:
        ...
        #Get task input
        task = input("Enter task(ping or toggle): ")
        if task =="ping":
...
def server(host, port, certfile, cafile=None):
    ...
    while True:
        ...
        client_thread = threading.Thread(target=handle_client, args=(raw_sock,addr, context))
        client_thread.start()

#handle_client is called by each thread to handle a single client connection
def handle_client(raw_sock, addr,context):
    # SSL wrapping
